...
class AdminStatus extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('users').doc(adminUserId).snapshots(),
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.exists) {
          ...
          if (isTyping) {
            return const Text("Admin is typing...");
          } else if (isOnline) {
            return const Text("Admin is Online");
          } else {
            if (lastSeen != null) {
              DateTime dateTime = lastSeen.toDate();
              String formattedDate = DateFormat('dd/MM/yyyy hh:mm a').format(dateTime);
              return Text("Admin last seen: $formattedDate)");
            } else {
              return const Text("Admin is Offline");
            }
          }
        } else {
          return const Text("Loading admin status...");
        }
      },
    );
  }
